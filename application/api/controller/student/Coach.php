...
class Coach extends Api
{
    ...
    public function coachlisttest()
    {
        ...
        $params['cooperation_id'] = '244';
        $params['student_type'] = 'student';
        $params['request_date'] = '2023-04-11';
    ...
    */
     ...
     * 接送车辆预约
     */
    public function pickupcarlist()
    {
        $params = $this->request->post();
        if(empty($params['request_date'])||empty($params['region'])){
            $this->error('参数缺失');
        }
        $datetime1_date = $params['request_date'];
        $city = $params['region'];
        ...
        foreach($res as $k=>$v){
            ...
            $list[$k]['coach'] = $this->getpickupcar($v['id']);
            if(!$list[$k]['coach']){
                unset($list[$k]);
                continue ;
            }
            foreach($list[$k]['coach'] as $kk=>$vv){
                //获取当前教员当天的请假情况
                $where_coach['pickup_id'] = $vv['id'];
                // $leavetime = $this->coachleave->where($where_coach)->select();
                $coach_reservetime =  Db::name('pickup_config_time')->where('pickup_id',$vv['id'])->select();
                
                $list[$k]['coach'][$kk]['timelist'] = $coach_reservetime;

                $coach_list = $this->pickup_order($list[$k]['coach'][$kk],$datetime1);
                $list[$k]['coach'][$kk]['timelist'] = $coach_list;
                unset($coach_list);
                unset($timelist);
                unset($leavetime);
    ...
    public function coachsctest()
    {
        ...
        $params['request_date'] = '2023-04-23';
    ...
    */
    public function pickupdetails(){
        $params = $this->request->post();
        // $params['pickupid']= 'CTN20220713170814623428';
        // $params['request_date'] =  '2022-08-11';
        // $params['stu_id'] =  'CSN20210425112607670801';
        // $params['student_type'] =  'student';
        if(empty($params['pickupid']) || empty($params['request_date']) || empty($params['stu_id']) || empty($params['student_type'])){
            $this->error('参数缺失');
        }
        ...
        //预约验证
        $this->reserve_pickup_validate($student,$student_type,$request_datetime,$request_datetime2);
        
        $pickupcar = $this->pickupcar->with('space')->where(['pickupcar.id'=>$pickupid])->find();
        $pickupcar['space']['pick_up_status'] = (int)$pickupcar['space']['pick_up_status'];

        // $pick_up_status = $pickupcar['space']['pick_up_status'];
        // $space = $this->space->where('id',$coach['space_id'])->find();
        $pickupcar = $this->getpickupinfo($pickupcar);
        // var_dump($coach_sc_keer,$coach_sc_kesan);exit;

        // $coo = $this->cooperation->where(['cooperation_id'=>$pickupcar['cooperation_id']])->find();
        ...
        //         }else{
        //             //redis获取场馆预约时间段如果有教员单独配置时间人数
        //             $reservetime = Db::name('coach_sc_config_time_people')->where('coach_id',$pickupcar['id'])->select();
        //             //获取教员当天的请假情况
        //             $where_coach['coach_id'] = $pickupid;
        //             $where_coach['starttime'] = ['>=',$request_datetime-(24*3600*7)];
        //             $where_coach['endtime'] = ['<',$request_datetime2+(24*3600*7)];
        //             $leavetime = $this->coachscleave->where($where_coach)->select();
        //             $timelist = $reservetime;
        ...
        //                 $timelist = $this->get_time($reservetime,$leavetime,$request_date);
        //             }
        //             // $open_time = Db::name('open_time')->where(['coach_id'=>$pickupcar['id'],'starttime'=>['between',[$request_datetime,$request_datetime2]]])->select();
        //             $open_time = [];

        //             $coach['detail_time_list'] = $this->get_sc_detail_time_list($timelist,$reservetime,$request_date,$pickupcar,$open_time);
        ...
        $reservetime = Db::name('pickup_config_time')->where('pickup_id',$pickupcar['pickupid'])->select();
        //获取教员当天的请假情况
        // $where_pickup['pickupid'] = $pickupid;
        // $where_pickup['starttime'] = ['>=',$request_datetime-(24*3600*7)];
        // $where_pickup['endtime'] = ['<',$request_datetime2+(24*3600*7)];
        // $leavetime = $this->coachscleave->where($where_pickup)->select();
        $timelist = $reservetime;
        // if($leavetime){
        //     $timelist = $this->get_time($reservetime,$leavetime,$request_date);
        // }
        // $open_time = Db::name('open_time')->where(['coach_id'=>$pickupcar['id'],'starttime'=>['between',[$request_datetime,$request_datetime2]]])->select();
        $open_time = [];
        $pickupcar['detail_time_list'] = $this->get_up_detail_time_list($timelist,$reservetime,$request_date,$pickupcar,$open_time);

        $this->success('返回成功',$pickupcar);
    }
    ...
    public function reserve_pickup_validate($student,$student_type,$request_datetime,$request_datetime2)
    {
        if($student_type == 'student'){
            $where['stu_id'] = $student['stu_id'];
            $where['reserve_starttime'] = ['between',[$request_datetime,$request_datetime2]];
            $where['order_status'] = 'paid';
            $order = $this->pickuporder->where($where)->find();
            if($order){
                $this->error('当日已有订单，无法重复预约');
            }
        }else{
            $this->error('当前不支持非正式学员');
        }
    }
    ...
    public function get_up_detail_time_list($timelist,$reservetime,$request_date,$pickupcar,$open_time){
        $time = array_column($open_time,'starttime');
        ...
        foreach($reservetime as $k=>$v){
            ...
            if(in_array($v,$timelist)){
                ...
                $number = $v['number'];
                $where['pickup_id'] = $pickupcar['pickupid'];
                $where['reserve_starttime'] = strtotime($Day.$v['starttimes']);
                $where['order_status'] = ['neq','cancel_refunded'];
                $order_number = $this->pickuporder->where($where)->count();
                $list[$k]['empty_num'] = $number-$order_number;
            }else{
                $list[$k]['empty_num'] = 0;
            }
    ...
    public function getpickupinfo($pickupcar){
        $info['pickupid'] = $pickupcar['id'];
        $info['machine_code'] = $pickupcar['machine_code'];
        $info['regionimage'] = $pickupcar['regionimage'];
        $info['space_id'] = $pickupcar['space_id'];
        $info['cooperation_id'] = $pickupcar['cooperation_id'];
        $info['state'] = $pickupcar['state'];
        $info['day'] = $pickupcar['space']['day'];
        $info['lng'] = $pickupcar['space']['lng'];
        $info['lat'] = $pickupcar['space']['lat'];
        $info['address'] = $pickupcar['space']['address'];
        $info['name'] = $pickupcar['space']['space_name'];
    ...
    function getpickupcar($space_id){
        $where['space_id'] = $space_id;
        $where['state'] = '1';
        $coach = $this->pickupcar->where($where)->field(['id','machine_code as name','cooperation_id','regionimage','state'])->select();
     ...
     * 教员当天所有时间段订单情况
     */
    function pickup_order($coach,$request_datetime){
        $count = 0;
        $num  = 0;
        $Day = $request_datetime;
        $timelist = $coach['timelist'];
        foreach($coach['timelist'] as $k=>$v){
            ...
            $where['reserve_starttime'] =  $starttime;
            $where['pickup_id'] = $coach['id'];
            $where['order_status'] = ['neq','cancel_refunded'];
            // $where['car_type'] = $car_type;
            $order_count = $this->pickuporder->where($where)->count();
            $count += $order_count;
            $people_upper = $people_upper-$order_count;
            $timelist[$k]['number'] = $people_upper;
