...
class Pickup extends Api
{
    ...
    {
        $where['pickup.status'] = '0';
        $res = $this->ordersc->with(['pickup'])->where($where)->select();
     ...
     * 获取接送教员
     */
    public function get_coach_pickup()
    {
        $params = $this->request->post();
        $params['space_id_list'] = 84;
        if(empty($params['space_id_list'])){
           $this->error('参数缺失');
        }

        $space = $this->space->where(['id'=>$params['space_id_list']])->find();
        $where['cooperation_id'] = $space['cooperation_id'];
        $where['teach_state'] = 'yes';
        $pickup_coachlist = Db::name('coach_pickup')->where($where)->field(['name','coach_id'])->select();
        if($pickup_coachlist){
            $this->success('返回成功',$pickup_coachlist);
        }else{
            $this->error('当前没有接送教练，请添加接送教练');
        }
    }
    ...
    public function time_list(){
        $params = $this->request->post();
        // $params['space_id_list'] = '84';
        // $params['request_date'] = '2022-12-22';
        ...
        $where1['order.space_id'] = $space_id_list;
        ...
        $where2['ordersc.space_id'] = $space_id_list;
        ...
        $where['cooperation_id'] = $space['cooperation_id'];
        $where['teach_state'] = 'yes';
        $pickup_coachlist = Db::name('coach_pickup')->where($where)->field(['name as title','coach_id as value'])->select();

       
        // $list['9:00-10:00'] = $aaa;
        $data['student'] = $list;
        $data['pickup_coachlist'] = $pickup_coachlist;
    ...
    public function choose_time()
    {
        ...
        $where['coach_id'] = ['not in',$coach];
        $coach_pickup = Db::name('coach_pickup')->where($where)->field(['name as title','coach_id as value'])->select();

        $data['coach_pickup'] = $coach_pickup;
        $data['list'] = $list;
        $this->success('返回成功',$data);
    }
