...
class Command extends Backend
{
    ...
    */
    ...
    {   $cooperation_list = $_SESSION['think']['admin']['cooperation_list'];
        $where['id'] = ['in',$cooperation_list];
        $cooperation = Model('Admin')->where($where)->field(['id','nickname'])->select();
        $list = [];
        foreach($cooperation as $k=>$v){
            $list[$k]['cooperation_id'] = $v['id'];
            $list[$k]['cooperation_name'] =$v['nickname'];
        }
        return $list;
    }
