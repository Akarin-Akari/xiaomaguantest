 ...
 * @icon fa fa-circle-o
 */
class Machineaimonth extends Backend
{

    /**
     * Machineaimonth模型对象
     * @var \app\admin\model\Machineaimonth
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Machineaimonth;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $where_cooperation['machineaimonth.cooperation_id'] = ['in', $this->cooperation_list];
            $list = $this->model
                ->with(['admin', 'space'])
    ...
    public function detail1($ids)
    {
        ...
        for ($i = 1; $i <= $day; $i++) {
            ...
            $count = Db::name('order_sc')->where($where)->count();
