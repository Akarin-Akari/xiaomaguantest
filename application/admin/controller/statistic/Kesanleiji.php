 ...
 * @icon fa fa-circle-o
 */
class Kesanleiji extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Kesanleiji;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $where_cooperation['kesanleiji.cooperation_id'] = ['in',$this->cooperation_list];
            $where1['kesanleiji.kesan_leiji'] = ['<>','NULL'];
            $list = $this->model
            ...
            foreach ($list as $row) {
				...
				$row->getRelation('warnexam')->visible(['kesan_leiji']);
            }
            foreach($list as $k=>$v){
