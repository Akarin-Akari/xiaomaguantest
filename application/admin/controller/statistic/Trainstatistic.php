...
use app\admin\model\Studyprocess;
 ...
 * @icon fa fa-circle-o
 */
class Trainstatistic extends Backend
{
    
    /**
     * Trainstatistic模型对象
     * @var \app\admin\model\Trainstatistic
     */
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $where_cooperation['trainstatistic.cooperation_id'] = ['in',$this->cooperation_list] ;
            ...
            foreach ($list as $row) {
                $row->visible(['id','total_order','stu_id','keer_hege','keer_leiji','kesan_hege','kesan_leiji','createtime']);
                $row->visible(['admin']);
            ...
            foreach($list as $k=>$v){
                $list[$k]['keer_leiji'] = $v['keer_leiji'] == 0 ? 0:number_format($v['keer_leiji']/3600,3);
                $list[$k]['kesan_leiji'] = $v['kesan_leiji'] == 0 ? 0:number_format($v['kesan_leiji']/3600,3);
            }
    ...
    public function detail1($ids)
    {
        ...
        $keer_study_time = $this->studyprocess->where(['stu_id'=>$row['stu_id'],'cooperation_id'=>$row['cooperation_id'],'createtime'=>['<',$starttime]])->whereNull('place_id')->select();
        $arr = [];
        $arr['xinshou'] = 0;
        $arr['jichubj'] = 0;
        $arr['fangxiangpan'] = 0;
        ...
        // var_dump($keys);exit;
        $num = 0;
        // var_dump($keer_study_time->toArray());exit;
    ...
    public function detail2($ids)
    {
        $starttime = \fast\Date::unixtime('day',0);
        $row = $this->model->get($ids,'student');
        $keer_study_time = $this->studyprocess->where(['stu_id'=>$row['stu_id'],'place_id'=>['<>','NULL'],'cooperation_id'=>$row['cooperation_id'],'createtime'=>['<',$starttime]])->select();
        $arr = [];
