...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        $uid = $admin['id'];
        $pid = $admin['pid'];
        $group_id = Db::name('AuthGroupAccess')->where('uid',$uid)->find()['group_id'];
        //获取分组的类型
        $group_type = Db::name('AuthGroup')->where('id',$group_id)->find()['group_type'];
        $space_list = [];
        $space_admin_list = [];
        $cooperation_list = [];
        $list =[];
        ...
        $_SESSION['think']['admin']['admin_title'] = '';

        //根据不同分组类型获取所属场馆
        if(in_array($group_type,[11,21,22,23])){
            //超级权限获取所有场馆
            $root_child_list = Db::name('Admin')->where('pid',1)->column('id');
            $child_list = Db::name('Admin')->where('pid','in',$root_child_list)->column('id');
            $list = Db::name('Space')->where('space_admin_id','in',$child_list)->select();

        }elseif(in_array($group_type,[24])){
            //合作方获取下方所有场馆id
            array_push($cooperation_list,$uid);
            $child_list = Db::name('Admin')->where('pid','in',$cooperation_list)->column('id');
            $list = Db::name('Space')->where('space_admin_id','in',$child_list)->select();
            $_SESSION['think']['admin']['admin_title'] = $user['admin_title'];


        }elseif(in_array($group_type,[31,32,33,35,36])){
            //合作方管理员，销售，财务，获取下方所有场馆id
            array_push($cooperation_list,$pid);
            $child_list = Db::name('Admin')->where('pid','in',$cooperation_list)->column('id');
            $list =Db::name('Space')->where('space_admin_id','in',$child_list)->select();
            $_SESSION['think']['admin']['admin_title']= Db::name('Admin')->where('id','in',$cooperation_list[0])->find()['admin_title'];

        }elseif(in_array($group_type,[34])){
            array_push($cooperation_list,$pid);
            $child_list = Db::name('Admin')->where('id',$uid)->column('id');
            $list = Db::name('Space')->where('space_admin_id','in',$child_list)->select();
            $_SESSION['think']['admin']['admin_title']= Db::name('Admin')->where('id','in',$cooperation_list[0])->find()['admin_title'];

            // var_dump($cooperation_list,$space_list,$space_admin_list);exit;
        }elseif(in_array($group_type,[41,42,43])){
            array_push($cooperation_list,$uid);
            //场馆管理员，销售，财务，获取场馆id
            $list = Db::name('Space')->where('space_admin_id',$usr_pid)->field(['id','cooperation_id','space_admin_id'])->select();
            $_SESSION['think']['admin']['admin_title']= Db::name('Admin')->where('id','in',$cooperation_list[0])->find()['admin_title'];

        }
        foreach($list as $v){
            array_push($space_list,$v['id']);
            array_push($space_admin_list,$v['space_admin_id']);
            array_push($cooperation_list,$v['cooperation_id']);
        }
        $cooperation_list = array_unique($cooperation_list);
        $cooperation_list = array_merge($cooperation_list);
        $_SESSION['think']['admin']['space_list'] = $space_list;
        $_SESSION['think']['admin']['cooperation_list'] = $cooperation_list;
        $_SESSION['think']['admin']['group_type'] = $group_type;
        $_SESSION['think']['admin']['space_admin_list'] = $space_admin_list;
