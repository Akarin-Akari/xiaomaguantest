...
class Pickuporder extends Model
{
    ...
    {
        $value = $value ? $value : (isset($data['pickup_time']) ? $data['pickup_time'] : '');
        return is_numeric($value) ? date("Y-m-d H:i:s", $value) : $value;
    }
    ...
    protected function setPickupTimeAttr($value)
    {
        return $value === '' ? null : ($value && !is_numeric($value) ? strtotime($value) : $value);
    }
    ...
    {
        return $this->belongsTo('Pickupcar', 'pickup_id', 'id', [], 'LEFT')->setEagerlyType(0);
    }
}
